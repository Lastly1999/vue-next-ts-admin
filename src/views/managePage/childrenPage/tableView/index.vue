  ...
  <div style="background: #fff; padding: 30px">
    ...
    <a-row style="margin-bottom: 10px">
      ...
      <a-col style="margin-left: 10px">
        <a-button type="primary" @click="appendList"> 添加 </a-button>
      </a-col>
    </a-row>
    <!-- 列表内容 -->
    <tableList
...
import {
  changeTalbeList,
  getAllTableList,
} from "@/service/api/tableHttp/index"; // 导入请求
...
export default defineComponent({
  ...
  setup() {
    const notice = new AntdNotice(); // 提示模块
    /**
     * 表格操作类
     */
    const tableListClass: TableListPerson = reactive<TableListPerson>({
      ...
      getTableList: async () => {
        ...
        const { code, data }: any = await getAllTableList(
          paginationClass.params
        );
        if (code === 200) {
    ...
    const fromDataClass = reactive({
      ...
      ruleFromSumbit: async () => {
        ...
        if (code === 200) {
          notice.showNotice("success", "修改成功", "");
    ...
    return {
      ...toRefs(tableListClass),
      ...toRefs(paginationClass),
      ...toRefs(modalClass),
      ...toRefs(fromDataClass),
      ruleFrom,
      onSearch,
    };
  },
